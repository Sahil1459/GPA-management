...
class Database {
protected:
    sql::Connection* con;  // Connection object

public:
    try {
        sql::mysql::MySQL_Driver* driver = sql::mysql::get_mysql_driver_instance();
        con = driver->connect("tcp://127.0.0.1:3306", "root", "password");  // Modify with your DB details
        con->setSchema("your_database_name");  // Use the correct database
    }
...
class admin : public common_variables, public Database { //This class handles all the functions for the admin and holds variables for the admin
...
class teacher : public common_variables, public Database {
...
public: void teachermenu() {
    ...
    if (choice == 1) {
      ...
      bool loginTeacher(string& emailid, string& password) {
          ...
          try {
			  unique_ptr<sql::PreparedStatement> pstmt(con->prepareStatement("SELECT password, department, fname, lname FROM teachers WHERE emailid = ?"));
      ...
      void viewMyclass() {
          ...
          if (choice == 1) {
              try {
                  unique_ptr<sql::PreparedStatement> pstmt(con->prepareStatement("Select enrollmentno,fname,mname,lname,mobile,gender,guardian_name,guardian_contact from students;"));

                  cout << "First Year class" << endl;
              }
              catch (sql::SQLException& e) {
              ...
              cout << "Second Year class" << endl;
          }
          else if (choice == 3) {
              cout << "Third Year class" << endl;
          }
      }
};
